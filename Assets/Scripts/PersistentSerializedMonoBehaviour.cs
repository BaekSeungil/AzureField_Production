...
public class PersistentSerializedMonoBehaviour<T> : SerializedMonoBehaviour
    where T : SerializedMonoBehaviour
{
    ...
    //  이 클래스는 DontDestroyOnLoad에 포함되어 씬이 바뀌더라도 제거되지 않습니다!!
    ...
    {
        if (instance == null) { instance = this as T; debug_static_objcect = gameObject.name; DontDestroyOnLoad(gameObject); }
