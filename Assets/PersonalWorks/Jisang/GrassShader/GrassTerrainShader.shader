Shader "Custom/BotWGrass"
{
    Properties
    {
        ...
        _BladeWidthMin ("Blade Width (Min)", Range(0, 0.2)) = 0.02
        _BladeWidthMax ("Blade Width (Max)", Range(0, 0.2)) = 0.05
        ...
        _TessellationGrassDistanceNear ("Tessellation Grass Distance (Near)", Range(0.1, 500)) = 0.5
        _TessellationGrassDistanceFar ("Tessellation Grass Distance (Far)", Range(0.001, 500)) = 0.5

        _TessellationGrassDistanceRate ("Tessellation Grass Distance (Rate)", Range(0.01, 10)) = 0.1
    ...
    SubShader
    {
        ...
        float _TessellationGrassDistanceNear;
        ...
        {
            float4 vertexPos = lerp(vert0.vertex, vert1.vertex, 0.5f);
            float d = distance(vertexPos.xyz, _WorldSpaceCameraPos) - _TessellationGrassDistanceNear;
            float depth = saturate(1 - (d / _TessellationGrassDistanceFar));

            //distance -> close is 1. far is 0
            float defaultDepth = depth;
            for (int i = 0; i < _TessellationGrassDistanceRate; ++i)
            {
                depth *= defaultDepth;
            }

            depth = ceil(depth * 5) * 0.2f;
            

            float3 v0 = vert0.vertex.xyz;
            float3 v1 = vert1.vertex.xyz;
            float edgeLength = distance(v0, v1) / _TessellationGrassDistance ;

            //small is large interval
            return edgeLength * depth ;
        }
        ...
        {
            GeomData o;
            // offset = mul(UNITY_MATRIX_M, offset);
        ...
        void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream)
        {
            ...
            if (grassVisibility >= _GrassThreshold)
            {
                ...
                for (int i = 0; i < BLADE_SEGMENTS; ++i)
                {
                    ...
                    triStream.Append(TransformGeomToClip(mspos, float3(offset.x, offset.y, offset.z), transformationMatrix, float2(0, t)));
                    triStream.Append(TransformGeomToClip(mspos, float3(-offset.x, offset.y, offset.z), transformationMatrix, float2(1, t)));
                }

                // Add the final vertex at the tip of the grass blade.
                triStream.Append(TransformGeomToClip(mspos, float3(0, forward, height), tipTransformationMatrix, float2(0.5, 1)));
        ...
        Pass
        {
            ...
            float4 frag(GeomData i) : SV_Target
            {
                ...
                #if defined(_MAIN_LIGHT_SHADOWS) || defined(_MAIN_LIGHT_SHADOWS_CASCADE) || true
