...
namespace JJS.Utils
{
    ...
    public static class ColorExtension
    {
        ...
        public static Color MultiplyToAlpha(this Color color, float value)
        {
            return new Color(color.r, color.g, color.b, color.a * value);
        }

        public static void ApplyAlphaToGradient(this Gradient gradient, float alpha)
        {
            GradientColorKey[] colorKeys = gradient.colorKeys;
            for (int i = 0; i < colorKeys.Length; i++)
            {
                colorKeys[i].color = colorKeys[i].color.MultiplyToAlpha(alpha);
            }
